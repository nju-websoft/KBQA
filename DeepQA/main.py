...
def train():
    print("重新训练，请保证计算机拥有至少4G空闲内存")
    # 准备训练数据
    print("正在加载训练数据...")
    with open("./data/train_sample.pkl", "rb") as f:  # 加载训练数据
        trainsample = list(pickle.load(f))  # 训练集大小75618
    tqs, tta, tfa = trainsample[0], trainsample[1], trainsample[2]  # 问题、正确三元组、错误三元组
    tqsl, ttal, tfal = trainsample[3], trainsample[4], trainsample[5]  # 问题长度、正确三元组长度、错误三元组长度
    print("加载完成！")
    print("正在加载验证数据...")
